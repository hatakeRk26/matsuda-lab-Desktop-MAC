...
# RSSI人数推定
# =========================
def estimate_by_rssi(sta_macs):

    count = 0

    for mac, rssi in sta_macs:
...
def extract_macs(pcap_file):
    ...
    if rssi_estimate_var.get():

        estimate_by_rssi([
            (mac, int(sum([r for r in data["rssi_list"] if r is not None]) /
                      len([r for r in data["rssi_list"] if r is not None])))
            for mac, data in sta_records.items()
            if any(r is not None for r in data["rssi_list"])
        ])
...
def start_capture():
    ...
    subprocess.run(
        ["iw", "dev", INTERFACE, "set", "channel", str(selected_channel)],
